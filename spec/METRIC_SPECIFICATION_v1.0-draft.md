...
The synthetic v0.1 wire compatibility profile accepts a decimal token only when its decimal value equals the shortest round-trip decimal spelling of its finite IEEE754 binary64 parse. Otherwise the adapter MUST reject it explicitly, not silently round it: `0.1` is accepted as1/10, but `0.10000000000000001` is rejected. This is a restricted input profile, not arbitrary-precision decimal input support. Integer tokens retain integer identity. A future general decimal wire format requires a new profile. The public Python wire parser and JavaScript reference enforce this check; callers supplying already-parsed language objects are responsible for preserving the original tokens or validating them before construction.
...
Wire canonicalization uses UTF-8 bytes containing ASCII JSON, no insignificant whitespace, lowercase `\\u` escapes for non-ASCII/DEL code units (supplementary characters use surrogate pairs), and JSON escapes for controls, quotes and backslashes. Integer tokens emit base10 without a fractional suffix. Accepted floating tokens emit the shortest round-trip decimal, preserving floating identity (`1.0`, not integer `1`); use scientific notation for nonzero magnitudes below1e-4 or at least1e16, a lowercase `e`, explicit exponent sign and at least two exponent digits. Float negative zero emits `-0.0`. These v0.1 compatibility bytes deliberately distinguish integer1 from floating1.0 even though their metric weights agree; this design is a documented candidate for a future normalized-number profile, not an unspecified language default. Duplicate object properties MUST be rejected at the protocol boundary.
